...
// src/handlers/get-player-by-id.js
var get_player_by_id_exports = {};
__export(get_player_by_id_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_player_by_id_exports);

// src/services/response.js
var import_http_status_codes2 = __toESM(require_cjs());

// src/errors/internal-server.js
var import_http_status_codes = __toESM(require_cjs());
var InternalServerError = class extends Error {
  constructor(message, code = "INTERNAL_SERVER_ERROR") {
    super(message);
    this.code = code;
    this.statusCode = import_http_status_codes.default.INTERNAL_SERVER_ERROR;
  }
};
var internal_server_default = InternalServerError;

// src/services/response.js
var formatError = (err) => {
  let error = { ...err };
  if (!error.statusCode) {
    error = new internal_server_default("Unexpected error occured on the server. Please contact the team.");
  }
  if (error.code >= import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR) {
    console.error(error);
  } else {
    console.info(error);
  }
  return {
    ...
    body: JSON.stringify(
      {
        errors: error.data && error.data.length > 0 ? error.data : [{
          type: error.code,
...
var formatResponse = (data, error, httpCode, headers = {}) => {
  if (error) {
    return formatError(error);
  }
  return {
    statusCode: httpCode || import_http_status_codes2.StatusCodes.OK,
...
// src/services/tennis.js
var import_axios = __toESM(require_axios2());
var getPlayerById = async (id) => {
  const { data: { players } } = await (0, import_axios.default)({
    method: "get",
...
var NotFound = class extends Error {
  constructor(id, code = "NOT_FOUND") {
    ...
    this.code = code;
    this.statusCode = import_http_status_codes3.default.NOT_FOUND;
  }
};
...
var handler = async (event, context) => {
  ...
  try {
    ...
    if (!player) {
      throw new not_found_default(playerId);
    }
